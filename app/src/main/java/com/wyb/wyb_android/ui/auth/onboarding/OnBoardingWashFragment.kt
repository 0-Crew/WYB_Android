...
import com.wyb.wyb_android.databinding.FragmentOnboardingWashBinding
...
class OnBoardingWashFragment : ViewModelFragment<FragmentOnboardingWashBinding, AuthViewModel>(
    R.layout.fragment_onboarding_wash
) {
                ...
                if (email != null) {
                    ...
                    viewModel.authType.observe(viewLifecycleOwner) { type ->
                        when (type) {
                            ...
                            AuthType.SIGNUP -> findNavController().safeNavigate(R.id.actionOnBoardingWashToNickname)
                        }
                    }
                } else {
                    Log.e(TAG, "로그인 실패", error)
